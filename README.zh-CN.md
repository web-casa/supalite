...
SupaLite 是从 Supabase 全家桶里挑出来的精简子集，给独立开发者在
单台 VPS 上自托管小项目用 —— 保留大多数应用真正在用的，砍掉用不上的。

|                                          | Supabase Cloud                                  | 自托管 Supabase 全家桶              | **SupaLite**                       |
|------------------------------------------|-------------------------------------------------|-------------------------------------|------------------------------------|
| **部署**                                 | 注册账号                                        | 11+ 个容器，手工组装                | 一条命令                           |
| **包含服务**                             | DB + Auth + Storage + Edge + Realtime + Studio  | 同左                                | DB + Auth + Studio                 |
| **日常运维**（备份/密钥/日志）           | 官方 Dashboard                                  | 自己来（`docker exec` + 脚本）      | 内置管理面板                       |
| **托管位置**                             | Supabase 的基础设施                             | 你的 VPS                            | 你的 VPS                           |
| **起步成本**                             | 免费档 → $25/月起                               | VPS（$5–10/月）+ 你的时间           | VPS（$5–10/月）                    |
| **需要 Storage / Edge Functions / Realtime？** | ✅                                         | ✅                                   | ❌ —— 这种情况选另外两个           |

**选 Supabase Cloud**：你想零运维，也不在乎绑定供应商。
**选自托管 Supabase 全家桶**：你确实需要完整生态
（Storage、Edge Functions、Realtime）。
**选 SupaLite**：你的应用就是 Postgres + Auth + 一个前端，你想把
数据放自己机器上，又不想折腾 11 个容器。
